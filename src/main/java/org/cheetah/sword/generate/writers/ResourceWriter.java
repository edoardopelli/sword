...
public class ResourceWriter {
    ...
    public void write(Path outputDir, NameResolver resolver, TableModel table) {
        ...
        TypeSpec.Builder type = TypeSpec.classBuilder(resourceType)
        ...
        for (ColumnModel col : table.getColumns()) {
            // Default naming: same strategy used for entity/dto (snake_case -> camelCase).
            String fieldName = resolver.columnPropertyName(table, col);
            fieldName = uniqueFieldName(fieldName, usedFieldNames);
            usedFieldNames.add(fieldName);

            TypeName fieldType = resolveResourceFieldType(col);
        ...
        JavaFile javaFile = JavaFile.builder(resourceType.packageName(), type.build())
                .indent("    ")
                .build();
    ...
    private TypeName resolveResourceFieldType(ColumnModel col) {
        // json/jsonb => Map<String,Object> (same type across Entity/DTO/Resource as requested)
        if (isJsonColumn(col)) {
