...
public class WizardRunner implements CommandLineRunner {
    ...
    public WizardRunner(DbIntrospector dbIntrospector, YamlService yamlService, CodeGenerationService codeGenerationService) {
    ...
    public void run(String... args) {
        ...
        if (hasYaml) {
            ...
            String outputDir = p.ask("Output directory", spec.getGeneration() != null ? spec.getGeneration().getOutputDir() : "./generated-src");
            DbModel model = yamlService.toDbModel(spec);
            codeGenerationService.generateAll(Path.of(outputDir), spec.getModel().getBasePackage(), model);            return;
        }
        ...
        String password = p.ask("Password", "");

        DbConnectionSpec cs = DbConnectionSpec.builder()
                .dbType(dbType)
                .host(host)
                .port(port)
                .database(database)
        ...
        DbModel model = dbIntrospector.introspect(
                ds,
                blankToNull(catalog),
                blankToNull(schema),
                pattern,
        ...
        if (genCode) {
            codeGenerationService.generateAll(Path.of(outputDir), basePackage, model);
        }
    }
        ...
        for (YamlSpec.Table t : spec.getTables()) {
            ...
            for (YamlSpec.Column c : t.getColumns()) {
                tb.column(org.cheetah.sword.model.ColumnModel.builder()
                        ...
                        .jdbcType(java.sql.Types.VARCHAR) // TODO: optionally store jdbcType int in YAML too
